...
//  http://allseeing-i.com/QBASIHTTPRequest
//
...
@class QBASIDataDecompressor;

extern NSString *QBASIHTTPRequestVersion;
...
typedef enum _QBASIAuthenticationState {
    QBASINoAuthenticationNeededYet = 0,
    QBASIHTTPAuthenticationNeeded = 1,
    QBASIProxyAuthenticationNeeded = 2
} QBASIAuthenticationState;

typedef enum _QBASINetworkErrorType {
    QBASIConnectionFailureErrorType = 1,
    QBASIRequestTimedOutErrorType = 2,
    QBASIAuthenticationErrorType = 3,
    QBASIRequestCancelledErrorType = 4,
    QBASIUnableToCreateRequestErrorType = 5,
    QBASIInternalErrorWhileBuildingRequestType = 6,
    QBASIInternalErrorWhileApplyingCredentialsType = 7,
    QBASIFileManagementError = 8,
    QBASITooMuchRedirectionErrorType = 9,
    QBASIUnhandledExceptionError = 10,
    QBASICompressionError = 11

} QBASINetworkErrorType;


// The error domain that all errors generated by QBASIHTTPRequest use
...
extern unsigned long const QBASIWWANBandwidthThrottleAmount;

#if NS_BLOCKS_AVAILABLE

typedef void (^QBASIBasicBlock)(void);

typedef void (^QBASIHeadersBlock)(NSDictionary *responseHeaders);

typedef void (^QBASISizeBlock)(long long size);

typedef void (^QBASIProgressBlock)(unsigned long long size, unsigned long long total);

typedef void (^QBASIDataBlock)(NSData *data);

#endif

@interface QBASIHTTPRequest : NSOperation <NSCopying, Cancelable> {
    ...
    // The delegate - will be notified of various changes in state via the QBASIHTTPRequestDelegate protocol
    ...
    // Generally, you won't use this directly, but QBASINetworkQueue sets itself as the queue so it can proxy updates to its own delegates
    ...
    // Automatically set to true in QBASIFormDataRequests when using setFile:forKey:
    ...
    // Set to true when QBASIHTTPRequest automatically created a temporary file containing the request body (when true, the file at postBodyFilePath will be deleted at the end of the request)
    ...
    // If error code is = QBASIConnectionFailureErrorType (1, Connection failure occurred) - inspect [[error userInfo] objectForKey:NSUnderlyingErrorKey] for more information
    NSError *error;
    ...
    // If you are using Basic authentication and want to force QBASIHTTPRequest to send an authorization header without waiting for a 401, you must set this to (NSString *)kCFHTTPAuthenticationSchemeBasic
    NSString *authenticationScheme;

    // Realm for authentication when credentials are required
    NSString *authenticationRealm;

    // When YES, QBASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a server that requires authentication
    ...
    // When YES, QBASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a proxy server that requires authentication
    ...
    // If you set this and implement the method in your delegate, you must handle the data yourself - QBASIHTTPRequest will not populate responseData or write the data to downloadDestinationPath
    ...
    // Also see the comments in QBASINetworkQueue.h
    ...
    // Tells QBASIHTTPRequest not to delete partial downloads, and allows it to use an existing file to resume a download. Defaults to NO.
    ...
    // Details on the proxy to use - you could set these yourself, but it's probably best to let QBASIHTTPRequest detect the system proxy settings
    NSString *proxyHost;
    int proxyPort;

    // QBASIHTTPRequest will assume kCFProxyTypeHTTP if the proxy type could not be automatically determined
    ...
    // See QBASIAuthenticationState values above. 0 == default == No authentication needed yet
    QBASIAuthenticationState authenticationNeeded;

    // When YES, QBASIHTTPRequests will present credentials from the session store for requests to the same server before being asked for them
    ...
    // Temporarily set to YES when a closed connection forces a retry (internally, this stops QBASIHTTPRequest cleaning up a temporary post body)
    ...
    // Will be QBASIHTTPRequestRunLoopMode for synchronous requests, NSDefaultRunLoopMode for all other requests
    NSString *runLoopMode;

    // This timer checks up on the request every 0.25 seconds, and updates progress
    NSTimer *statusTimer;

    // The download cache that will be used for this request (use [QBASIHTTPRequest setDefaultCache:cache] to configure a default cache
    id <QBASICacheDelegate> downloadCache;

    // The cache policy that will be used for this request - See QBASICacheDelegate.h for possible values
    QBASICachePolicy cachePolicy;

    // The cache storage policy that will be used for this request - See QBASICacheDelegate.h for possible values
    QBASICacheStoragePolicy cacheStoragePolicy;
    ...
    //
    // Setting this to NO may be especially useful for users using QBASIHTTPRequest in conjunction with a streaming parser, as it will allow partial gzipped responses to be inflated and passed on to the parser while the request is still running
    ...
    QBASIBasicBlock startedBlock;

    //block to execute when headers are received
    QBASIHeadersBlock headersReceivedBlock;

    //block to execute when request completes successfully
    QBASIBasicBlock completionBlock;

    //block to execute when request fails
    QBASIBasicBlock failureBlock;

    //block for when bytes are received
    QBASIProgressBlock bytesReceivedBlock;

    //block for when bytes are sent
    QBASIProgressBlock bytesSentBlock;

    //block for when download size is incremented
    QBASISizeBlock downloadSizeIncrementedBlock;

    //block for when upload size is incremented
    QBASISizeBlock uploadSizeIncrementedBlock;

    //block for handling raw bytes received
    QBASIDataBlock dataReceivedBlock;

    //block for handling authentication
    QBASIBasicBlock authenticationNeededBlock;

    //block for handling proxy authentication
    QBASIBasicBlock proxyAuthenticationNeededBlock;

    //block for handling redirections, if you want to
    QBASIBasicBlock requestRedirectedBlock;
#endif
}
...
+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <QBASICacheDelegate>)cache;

+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <QBASICacheDelegate>)cache andCachePolicy:(QBASICachePolicy)policy;

#if NS_BLOCKS_AVAILABLE
- (void)setStartedBlock:(QBASIBasicBlock)aStartedBlock;

- (void)setHeadersReceivedBlock:(QBASIHeadersBlock)aReceivedBlock;

- (void)setCompletionBlock:(QBASIBasicBlock)aCompletionBlock;

- (void)setFailedBlock:(QBASIBasicBlock)aFailedBlock;

- (void)setBytesReceivedBlock:(QBASIProgressBlock)aBytesReceivedBlock;

- (void)setBytesSentBlock:(QBASIProgressBlock)aBytesSentBlock;

- (void)setDownloadSizeIncrementedBlock:(QBASISizeBlock)aDownloadSizeIncrementedBlock;

- (void)setUploadSizeIncrementedBlock:(QBASISizeBlock)anUploadSizeIncrementedBlock;

- (void)setDataReceivedBlock:(QBASIDataBlock)aReceivedBlock;

- (void)setAuthenticationNeededBlock:(QBASIBasicBlock)anAuthenticationBlock;

- (void)setProxyAuthenticationNeededBlock:(QBASIBasicBlock)aProxyAuthenticationBlock;

- (void)setRequestRedirectedBlock:(QBASIBasicBlock)aRedirectBlock;
#endif
...
// Used by QBASINetworkQueue to create a HEAD request appropriate for this request with the same headers (though you can use it yourself)
- (QBASIHTTPRequest *)HEADRequest;
...
// Used internally to record bandwidth use, and by QBASIInputStreams when uploading. It's probably best if you don't mess with this.
+ (void)incrementBandwidthUsedInLastSecond:(unsigned long)bytes;

// On iPhone, QBASIHTTPRequest can automatically turn throttling on and off as the connection type changes between WWAN and WiFi
...
+ (void)setDefaultCache:(id <QBASICacheDelegate>)cache;

+ (id <QBASICacheDelegate>)defaultCache;
...
+ (NSThread *)threadForRequest:(QBASIHTTPRequest *)request;
...
@property(assign) QBASIHTTPRequest *mainRequest;
...
@property(assign, readonly) QBASIAuthenticationState authenticationNeeded;
...
@property(assign) id <QBASICacheDelegate> downloadCache;
@property(assign) QBASICachePolicy cachePolicy;
@property(assign) QBASICacheStoragePolicy cacheStoragePolicy;
