 ...
 * 
 * Step 3:
 * Replace your NSLog statements with QBDDLog statements according to the severity of the message.
 * 
 * NSLog(@"Fatal error, no dohickey found!"); -> DDLogError(@"Fatal error, no dohickey found!");
 * 
 * QBDDLog works exactly the same as NSLog.
...
@class QBDDLogMessage;

@protocol QBDDLogger;
@protocol QBDDLogFormatter;
...
NSString *QBExtractFileNameWithoutExtension(const char *filePath, BOOL copy);

#define THIS_FILE (QBExtractFileNameWithoutExtension(__FILE__, NO))
...
+ (void)addLogger:(id <QBDDLogger>)logger;

+ (void)removeLogger:(id <QBDDLogger>)logger;
...
@protocol QBDDLogger <NSObject>
@required

- (void)logMessage:(QBDDLogMessage *)logMessage;
...
**/
- (id <QBDDLogFormatter>)logFormatter;

- (void)setLogFormatter:(id <QBDDLogFormatter>)formatter;
...
@protocol QBDDLogFormatter <NSObject>
@required
...
- (NSString *)formatLogMessage:(QBDDLogMessage *)logMessage;
...
@interface QBDDLogMessage : NSObject {
...
/**
 * The QBDDLogger protocol specifies that an optional formatter can be added to a logger.
...
@interface QBDDAbstractLogger : NSObject <QBDDLogger> {
    id <QBDDLogFormatter> formatter;
